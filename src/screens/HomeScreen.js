...
export default function HomeScreen({ navigation }) {
    ...
    const [users, resList] = useState([])

    useEffect(() => {
        const getData = async () => {
            const restaurantsData = collection(db, "restaurantsData")
            const resSnapshot = await getDocs(restaurantsData);
            const resItems = resSnapshot.docs.map(doc => doc.data());
            resList(resItems)
        }
        getData();
    }, []);
